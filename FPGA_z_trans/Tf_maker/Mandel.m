maxIterations = 500;
gridSizex = 1920*2;
gridSizey = 1080*2;
%xlim = [-0.748766713922161, -0.748766707771757];
%ylim = [ 0.123640844894862,  0.123640851045266];
centre = [(-0.748766713922161 + -0.748766707771757)/2, (0.123640844894862 + 0.123640851045266)/2];
yside = 6.1504e-08;
xside = yside * 1920/1080;
xlim = centre(1) + [-(xside/2), (xside/2)];
ylim = centre(2) + [-(yside/2), (yside/2)];

% Setup
t = tic();
x = gpuArray.linspace( xlim(1), xlim(2), gridSizex );
y = gpuArray.linspace( ylim(1), ylim(2), gridSizey );
[xGrid,yGrid] = meshgrid( x, y );
z0 = complex( xGrid, yGrid );
count = ones( size(z0), 'gpuArray' );

% Calculate
z = z0;
for n = 0:maxIterations
    z = z.*z + z0;
    inside = abs( z )<=2;
    count = count + inside;
end
count = log( count );

% Show
count = gather( count ); % Fetch the data back from the GPU
naiveGPUTime = toc( t );
imagesc( x, y, count )
axis off
%title( sprintf( '%1.3fsecs (naive GPU) = %1.1fx faster', ...
%    naiveGPUTime, cpuTime/naiveGPUTime ) )